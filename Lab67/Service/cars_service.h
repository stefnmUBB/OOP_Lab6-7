...
private:
	...
	vector<string> wash_list;
	stack<unique_ptr<UndoAction>> undo_list;
public:
	void debug_populate_random(size_t cnt);

	size_t wash_list_count() const noexcept; 


	void clear_wash_list() noexcept;

	void add_to_wash_list(string number_plate);

	void populate_wash_list(size_t count);
