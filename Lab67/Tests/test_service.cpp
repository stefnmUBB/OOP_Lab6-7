...
void test_service_wash_list_add()
{
	...
	assert(cs.wash_list_count() == 1);

	try { cs.add_to_wash_list("AA05ZZZ"); assert(false); } catch (car_already_in_wash_list_exception) { assert(true); }
	try { cs.add_to_wash_list("ZZ05ZZZ"); assert(false); } catch (car_not_found_exception) { assert(true); }
...
void test_service_wish_list_populate()
{
	...
	try { cs.populate_wash_list(10); assert(false); } catch (std::out_of_range) { assert(true); }

	cs.populate_wash_list(5);
	assert(cs.wash_list_count() == 5);
}
